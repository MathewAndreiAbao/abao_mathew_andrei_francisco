...
<body class="bg-gray-900 min-h-screen flex items-center justify-center text-gray-200">
    <div class="bg-gray-800 rounded-lg shadow-lg p-8 max-w-md w-full">
        ...
        <form action="<?= site_url('/notes/edit_post/' . $note['id']) ?>" method="post" class="space-y-4">
            <div>
            ...
            <div>
                ...
                <textarea id="content" name="content" rows="6" required
                    class="w-full px-3 py-2 rounded bg-gray-700 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500"><?= htmlspecialchars($note['content']) ?></textarea>
            </div>
            <button type="submit" class="w-full bg-gray-700 hover:bg-gray-600 text-white font-bold py-2 rounded transition">Update Note</button>
        </form>
